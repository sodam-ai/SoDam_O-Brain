...
export async function addMemory(db, { content, type = '지식', importance = 3, source = 'ai', confidence = 0.6, project = null }) {
  ...
  const tx = db.transaction(() => {                 // 3) 원자적 저장
    const id = Number(db.prepare(
      `INSERT INTO memory(content, type, importance, confidence, source, project) VALUES (?,?,?,?,?,?)`
    ).run(clean, type, importance, confidence, source, project).lastInsertRowid);
